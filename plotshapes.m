...
% PLOTSHAPES shape configuration overlay from configuration matrix or from
% Principal scores of PNS coordinates
...
switch isConfig
    case true % then input is array of configuration matrices
        ...
        if nargin > 2
            ...
            if isfield(paramstruct,'LineStyle') ;    %  then change to input value
                linestyle = paramstruct.LineStyle;
            end ;
        ...
        for i=1:n;
            X = data(:,:,i);
            Xmat((1:k)+(i-1)*n,:) = X;
            plot(X(joinline,1),X(joinline,2),'LineStyle',linestyle,'Color',icolor(i,:),'Marker',markerstr(i),'LineWidth',linewidth)
