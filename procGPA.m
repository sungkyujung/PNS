...
%    n : number of samples
%
% Sep 15, 2011 : fixed errors regarding eig.m 
% Jan 15, 2010
% 
% Sungkyu Jung
...
while diff1 > 1e-15
    ...
    RHO = corr(vecdata);
    [V,D] = eig(RHO);
    if real(D(1,1)) > real(D(end,end)) % choose the eigenvector with largest eigenvalue
        phi = V(:,1);
    else
        phi = V(:,end);
    end
    ...
    for i=1:n
        ...
        data(:,:,i) = abs(betai)*data(:,:,i); % betai is positive
    end
    Gnext = ssnpd(data);
    diff1 = Gnow - Gnext;
    Gnow = Gnext;
