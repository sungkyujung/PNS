...
%                          by an action in SO(m)) as geodesic mean of the
%                          preshapes.
%
...
%            the sequential test.
%
% use_parallel     false (default) do not use parfor in vMFtest
%                  true   use parfor in vMFtest.
%                         Parallel Computing Toolbox is required.
%                         You need to type "matlabpool open" in Matlab Command
%                         Window before running PNSshape.m
%                         When you finished running PNSshape, type
%                         "matlabpool close".
%               Parallel Computing mode thanks to Takayuki Iwamoto (Nov 7,
%               2013)
%
...
if nargin > 1 ;   %  then paramstruct is an argument
    ...
    if isfield(paramstruct,'use_parallel') ;    %  then change to input value
        g_use_parallel = paramstruct.use_parallel;
        %--check if Parallel Computing Toolbox is installed.
        tblist = ver;
        if( g_use_parallel == true )
            if any(strcmp('Parallel Computing Toolbox',{tblist.Name}))
                %--check if matlabpool is opened
                if (matlabpool('size') == 0)
                    fprintf('\n---------------------------------------------\n');
                    fprintf('PNSshape.m\n');
                    % fprintf('WARNING: matlabpool is not open. \n');
                    try 
                        matlabpool open;
                    catch
                        g_use_parallel = false; 
                ...
                error('Parallel Computing Toobox is not installed on your machine.\n');
            end
